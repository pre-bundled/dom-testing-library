...
function configure(newConfig) {
  ...
  config = _extends(_extends({}, config), newConfig);
}
function getConfig() {
  return config;
}
...
function getRoles(container, _temp) {
  ...
  return flattenDOM(container).filter(function (element) {
    ...
    return roles.reduce(function (rolesAcc, role) {
      ...
      return Array.isArray(rolesAcc[role]) ? _extends(_extends({}, rolesAcc), {}, (_extends2 = {}, _extends2[role] = [].concat(rolesAcc[role], [node]), _extends2)) : _extends(_extends({}, rolesAcc), {}, (_extends3 = {}, _extends3[role] = [node], _extends3));
    }, acc);
  }, {});
}
...
Object.keys(eventMap).forEach(function (key) {
  ...
  createEvent[key] = function (node, init) {
    ...
    var eventInit = _extends(_extends({}, defaultInit), init);
